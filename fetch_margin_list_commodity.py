...
def fetch_margin_data():
    url = "https://zerodha.com/margin-calculator/Commodity/"
    ...
    table = soup.find('table', {'id': 'table', 'class': 'data commodity'})
    ...
    for row in rows:
        data = {
            ...
            'lot_size': int(row.get('data-lot_size')),
            'price': float(row.get('data-price')),
            'nrml': {
                'margin_amount': int(row.get('data-nrml_margin')),
                'margin_percent': float(row.get('data-margin'))
            }
        }
    ...
    output_file = "margin_list_commodity.json"
...
if __name__ == "__main__":
    try:
        print(f"{datetime.now()} - Started margin list fetching commodity")
        fetch_margin_data()
        print(f"{datetime.now()} - Completed fetching & exporting margin list commodity")
        sys.exit(0)
